	...
	ModelAndView FacilityManager(HttpServletRequest request) {
		...
		if (operation != null) {
			if (operation.equals("viewUsers")) {
				...
				String isMeetingRoomAvailableURL ="isMeetingRoomAvailable/"+requestId;
				
				MeetingRequest meetingRequest = new MeetingRequest();
				meetingRequest.setRequestId(requestId);
				Boolean isMeetingRoomAvailable = restTemplate.getForObject(facilityManagerBaseURL + isMeetingRoomAvailableURL, Boolean.class);
				if(isMeetingRoomAvailable) {
				restTemplate.put(facilityManagerBaseURL + acceptRequestURL, meetingRequest);
